...
![](../files/tufte-books.jpg){width=700px}
...
```{r barplot_01,fig.width=6,fig.height=5}
p <- mtcars %>% group_by(cyl) %>% 
      summarise(mean_mpg=mean(mpg)) %>%
      mutate(cyl=factor(cyl)) %>% 
      ggplot(aes(x=cyl, y=mean_mpg, fill=cyl))
...
```{r barplot_02,fig.width=6,fig.height=5}
p + geom_bar(stat="identity", mapping=aes(fill=cyl)) + 
...
```{r barplot_03,fig.width=6,fig.height=5}
p + geom_bar(stat="identity", mapping=aes(fill=cyl)) + 
  theme(legend.position="none")
```
...
```{r barplot_04,fig.width=6,fig.height=5}
...
```{r barplot_05,fig.width=6,fig.height=5}
...
```{r barplot_06,fig.width=6,fig.height=5}
p + geom_bar(stat="identity", fill="#666666", width=0.5) + 
...
```{r barplot_07,fig.width=6,fig.height=5}
p + geom_bar(stat="identity", fill="#666666", width=0.5) + 
  theme(legend.position="none", panel.grid.major.x=element_blank(), 
...
```{r barplot_08,fig.width=6,fig.height=5}
p + geom_bar(stat="identity", fill="#666666", width=0.5) + 
  theme(legend.position="none", panel.grid.major.x=element_blank(), 
    panel.ontop=T, panel.background=element_rect(fill="#FFFFFF00"), 
    panel.grid.major.y=element_line(size=1, color="white"), 
...
```{r boxplot_02,fig.width=8,fig.height=5}
...
```{r scatter_themes,fig.width=8,fig.height=5}
p <- list()
...
```{r plotwidgets,fig.width=5,fig.height=5}
library(plotwidgets)
## Now loop over hues
...
```{r rcolorbrewer,fig.width=8,fig.height=5}
